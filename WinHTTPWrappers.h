...
#pragma warning(suppress: 26477 26490)
            ...
            HRESULT hr = ATL::AtlHresultFromWin32(pResult->dwError);
            // 取消时返回 E_ABOART
            if (dwInternetStatus == WINHTTP_CALLBACK_STATUS_HANDLE_CLOSING) {
                hr = E_ABORT;
            }
            //Call the OnCallbackComplete method with the async HRESULT
            return OnCallbackComplete(hr, hInternet, dwInternetStatus, lpvStatusInformation, dwStatusInformationLength);
        }
