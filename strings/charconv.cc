...
int BitWidth(uint128 value) {
  if (Uint128High64(value) == 0) {
    ...
    return static_cast<int>(std::bit_width(Uint128Low64(value)));
  }
  return 128 - std::countl_zero(Uint128High64(value));
}
...
CalculatedFloat CalculateFromParsedHexadecimal(
  ...
  // This static_cast is only needed when using a std::bit_width()
  // implementation that does not have the fix for LWG 3656 applied.
  int mantissa_width = static_cast<int>(std::bit_width(mantissa));
...
#else
  if constexpr (FloatTraits<FloatType>::kTargetBits == 64) {
